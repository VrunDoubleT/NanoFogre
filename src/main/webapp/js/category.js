...
//================================= Create Category =======================================////
        ...
        addAttributeForm();
                ...
                /////Valid number INT?Float?Dat
                function validateAttributeNumberFields(counter) {
                // Set data
                const datatype = document
                        .querySelector(`select[name="attributeDatatype_${counter}"]`)
                        ?.value;
                        // Invalid(int/float) skip
                ...
                if (!inp) return;
                        inp.parentElement.querySelector('.number-error-msg')?.remove();
                        inp.classList.remove('border-red-500', 'border-green-500', 'ring-1');
                });
                        ...
                        // 2)check negative and zero
                        if (nMin !== null && nMin < 0) {
                isValid = false;
                        minInput.classList.add('border-red-500', 'ring-1', 'ring-red-500');
                        const err = document.createElement('div');
                        err.className = 'text-red-500 text-xs mt-1 number-error-msg';
                        err.textContent = 'Min value cannot be negative.';
                        minInput.parentElement.appendChild(err);
                }
                if (nMax !== null && nMax <= 0) {
                isValid = false;
                        maxInput.classList.add('border-red-500', 'ring-1', 'ring-red-500');
                        const err = document.createElement('div');
                        err.className = 'text-red-500 text-xs mt-1 number-error-msg';
                        err.textContent = 'Max value must be greater than zero.';
                ...
                // 4) check Min < Max
                if (isValid && nMin !== null && nMax !== null && nMin >= nMax) {
                isValid = false;
                [minInput, maxInput].forEach(i => {
                i.classList.remove('border-green-500', 'ring-green-500');
                        i.classList.add('border-red-500', 'ring-1', 'ring-red-500');
                });
                        maxInput.parentElement.querySelector('.number-error-msg')?.remove();
                        const err = document.createElement('div');
                        err.className = 'text-red-500 text-xs mt-1 number-error-msg';
                        err.textContent = 'Min value must be less than Max value.';
                        maxInput.parentElement.appendChild(err);
                }
...
//////////////////////////////////////////
                ...
                const minInput = document.querySelector(`input[name="attributeMin_${counter}"]`);
                        ...
                        if (!minInput || !maxInput || !dtSelect) return true;
                        // show error
                ...
                let isValid = true;
                        ...
                        if (! validateAttributeNumberFields(counter)) {isValid = false; }
...
/////////ShowAttribute
                ...
                const dt = item.querySelector('select[name^="attributeDatatypeShow"]');
                        const mm = item.querySelector('.min-max-containerShow');
                        if (!dt || !mm)
                        return;
                        const saveState = () => {
                const data = {};
...
///// ======Check valid======///////
                        ///valid Date
                                ...
                                const minInput = item.querySelector('input.min-input[type="date"]');
                                        ...
                                        // 1) Xóa hết lỗi & highlight cũ
                                        [minInput, maxInput].forEach(inp => {
                                if (!inp) return;
                                        inp.parentElement.querySelector('.date-error-msg')?.remove();
                                        inp.classList.remove("border-red-500", "ring-1", "ring-green-500");
                                });
                                        // 2) Validate riêng từng ô: không trước 2000-01-01 và không quá ngày hôm nay
                                        [minInput, maxInput].forEach(inp => {
                                ...
                                isValid = false;
                                        ...
                                        inp.insertAdjacentHTML('afterend',
                                                `<div class="text-red-500 text-xs mt-1 date-error-msg">${msg}</div>`
                                                );
                                } else {
                                inp.classList.add("ring-1", "ring-green-500");
                                }
                                });
                                        // 3) Check Min < Max (cấm Min ≥ Max)
                                        if (
                                                minInput && maxInput &&
                                ...
                                inp.classList.remove("ring-green-500");
                                        ...
                                        minInput.insertAdjacentHTML('afterend',
                                                `<div class="text-red-500 text-xs mt-1 date-error-msg">
             Min date must be earlier than Max date
           </div>`
                                                );
                                        maxInput.insertAdjacentHTML('afterend',
                                                `<div class="text-red-500 text-xs mt-1 date-error-msg">
             Max date must be later than Min date
           </div>`
                                                );
                                }
                                }
...
//valid Name and unit
                        ...
                        if (!input)
                                return;
                                const parent = input.parentElement;
                                const oldError = parent.querySelector('.text-error-msg');
                                if (oldError)
                                oldError.remove();
                                input.classList.remove("border-red-500", "ring-1", "ring-green-500");
                                if (!input.value.trim()) {
                        isValid = false;
                                ...
                                error.className = "text-red-500 text-xs mt-1 text-error-msg";
                                error.textContent = (input === nameInput)
...
///======
                        ...
                        const dateValid = validateDateFields();
                                const numberValid = validateAttributeNumberFields();
                                const textValid = validateAttributeTextFields(); // 👈 MỚI

                                if (!dateValid || !numberValid || !textValid)
                                return;
                                nameError.textContent = "";
                        ...
                        const idEl = item.querySelector('[name="attributeId"]');
                                ...
                                const nameEl = item.querySelector('[name^="attributeName"]');
                                const dtEl = item.querySelector('[name^="attributeDatatype"]');
                                const unitEl = item.querySelector('[name^="attributeUnit"]');
                                const minEl = item.querySelector('[name^="attributeMin"]');
                                const maxEl = item.querySelector('[name^="attributeMax"]');
                                arr.push({
                                attributeId:   aid,
                                        attributeName: nameEl.value.trim(),
                                        datatype:      dtEl.value,
                                        unit:          unitEl.value.trim() || null,
                                        minValue:      minEl?.value.trim() || null,
                                        maxValue:      maxEl?.value.trim() || null,
                                        isRequired:    item.querySelector('[name^="attributeRequired"]').checked,
                                        isActive:      item.querySelector('[name^="attributeActive"]')?.value === "true"
                                });
                        }
                        );
                                fd.append("attributes", JSON.stringify(arr));
                                showLoading();
